...
module CriticalRoll =
    ...
    let createCritFunction (critValue: DiceValue option) =
        ...
        | None -> fCriticalRoll (DiceValue.six)
        | Some value -> fCriticalRoll value

module Rolls =

    let criticalRoll (fCrit: CriticalRollCheck) (diceRoll: DiceValue) =
        match fCrit diceRoll with
    ...
    let apply (feelNoPain: FeelNoPain) (damage: Damage) =
        ...
        let tryApply =
            (Dice.rollDice Dice.D6 ()
             |> Rolls.successfulRoll None (FeelNoPain.toRequiredRoll feelNoPain) DiceValueModifier.noModifier)
